...
cv::Rect get_rect(cv::Mat& img, float bbox[4], int InputH, int InputW);
...
void draw_bbox(cv::Mat& img, std::vector<Detection>& res, int InputH, int InputW);

void draw_bbox_keypoints_line(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch);

void batch_process(std::vector<std::vector<Detection>>& res, const float* decode_ptr_host, int batch_size,
                   int bbox_element, const std::vector<cv::Mat>& img);

void batch_process(std::vector<Detection>& res, const float* decode_ptr_host,
                   int bbox_element, const cv::Mat& image);
